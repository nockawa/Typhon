...
public static class Program
{
    ...
    public static void Main(string[] args)
    {
        ...
        if (traceFile != null)
        {
            Console.WriteLine($"Profiler: file mode → {traceFile}");
        }
        else if (livePort >= 0)
        {
            Console.WriteLine($"Profiler: live mode → TCP listener on port {livePort}");
        }
    ...
    private static void PrintUsage()
    {
        ...
        Console.WriteLine($"                         (default port: {ProfilerSetup.DefaultLivePort})");
