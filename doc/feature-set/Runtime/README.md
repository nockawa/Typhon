...
| [Typed Event Queues](typed-event-queues.md) | Multi-producer per-worker-segment queues for inter-system signalling within a tick; any number of parallel chunk workers may push concurrently via `ctx.Writer(queue)`, enabling reactive cascade chains that early-out cheaply when dormant | ✅ Implemented | 🟣 Advanced |
