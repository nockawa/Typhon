...
public readonly struct SpatialGridAccessor
{
    ...
    public int GridWidth => _grid.Config.GridWidth;

    /// <summary>Grid height in cells.</summary>
    public int GridHeight => _grid.Config.GridHeight;

    /// <summary>Cell size in world units.</summary>
    public float CellSize => _grid.Config.CellSize;
    ...
    internal ref CellDescriptor GetCell(int cellKey) => ref _grid.GetCell(cellKey);
    ...
    internal ref CellDescriptor GetCell(int cellX, int cellY) => ref _grid.GetCell(_grid.ComputeCellKey(cellX, cellY));
    ...
    public void SetCellTier(int cellX, int cellY, SimTier tier) => _grid.SetCellTier(_grid.ComputeCellKey(cellX, cellY), tier);
    ...
    /// </summary>
    public void SetCellTierMin(int cellX, int cellY, SimTier tier) => _grid.SetCellTierMin(_grid.ComputeCellKey(cellX, cellY), tier);

    /// <summary>Bulk-set all cells to the specified tier. Typically called at the start of <c>TierAssignment</c>.</summary>
    public void ResetAllTiers(SimTier tier) => _grid.ResetAllTiers(tier);
    ...
    public int WorldToCell(float worldX, float worldY) => _grid.WorldToCellKey(worldX, worldY);

    /// <summary>Convert a cell key back to grid coordinates (x, y).</summary>
    public (int x, int y) GetCellCoords(int cellKey) => _grid.CellKeyToCoords(cellKey);
}
