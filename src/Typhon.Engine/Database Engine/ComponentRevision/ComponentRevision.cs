...
internal ref struct ComponentRevision : IDisposable
{
    ...
    internal ComponentRevision(Transaction.ComponentInfoBase info, ref Transaction.ComponentInfoBase.CompRevInfo compRevInfo, int firstChunkId, ChunkRandomAccessor accessor)
    {
        _accessor = accessor;
        _info = info;
    ...
    internal ComponentRevisionManager.ElementRevisionHandle GetRevisionElement(short revisionIndex) => ComponentRevisionManager.GetRevisionElement(_accessor, _firstChunkId, revisionIndex);    
    ...
    internal bool CleanUpUnusedEntries(long nextMinTSN) => ComponentRevisionManager.CleanUpUnusedEntries(_info, ref _compRevInfo, _accessor, nextMinTSN);
