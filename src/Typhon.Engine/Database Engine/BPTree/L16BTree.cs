...
namespace Typhon.Engine.BPTree
{
    ...
    public abstract class L16BTree<TKey> : BTree<TKey> where TKey : unmanaged
    {
        unsafe public class L16NodeStorage : BaseNodeStorage
        {
            ...
            {
                ref var chunk = ref ChunkAccessor.GetChunk<Index16Chunk>(node.ChunkId, true);
            ...
            {
                ref readonly var chunk = ref ChunkAccessor.GetChunkReadOnly<Index16Chunk>(node.ChunkId);
            ...
            {
                ref var leftChunk = ref ChunkAccessor.GetChunk<Index16Chunk>(left.ChunkId, true);
                ref var rightChunk = ref ChunkAccessor.GetChunk<Index16Chunk>(right.ChunkId, true);
                ...
                if (leftChunk.Count + right.Count > Index16Chunk.Capacity)
                ...
                {
                    bool copyIsOnePiece = end + right.Count <= Index16Chunk.Capacity;
            ...
            {
                if (length == 0) return;
                if (length < 0 || length > Index16Chunk.Capacity) throw new ArgumentOutOfRangeException(nameof(length));
                if (index < 0 || index >= Index16Chunk.Capacity) throw new ArgumentOutOfRangeException(nameof(length));
            ...
            public NodeWrapper SplitRight(ref Index16Chunk left, NodeStates states)
            {
                var rightNode = Owner.AllocNode(states);
                ref var right = ref ChunkAccessor.GetChunk<Index16Chunk>(rightNode.ChunkId, true);
