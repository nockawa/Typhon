...
/// <summary>
/// Tick phase enum — mirrored one-for-one (same numeric values) from <see cref="Typhon.Engine.TickPhase"/> so the runtime's scheduler can
/// reinterpret-cast bytes directly to this enum when emitting profiler events. Lives in the profiler namespace to keep the
/// <c>Typhon.Engine.Profiler.Events</c> API surface self-contained.
/// </summary>
